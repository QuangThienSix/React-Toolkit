...
const studentSlice = createSlice({
  ...
  initialState,
  reducers: {
    fetchStundetList(state, action: PayloadAction<ListParams>) {
      state.loading = true;
    },
    fetchStundetListSusscess(state, action: PayloadAction<ListResponse<Student>>) {
      state.list = action.payload.data;
      state.pagination = action.payload.pagination;
      state.loading = false;
    },
    fetchStundetListFailed(state) {
      state.loading = false;
    },
